...
TOOLS = [
    ...
    tool("draw_stroke",
         ...
         {"document": DOCUMENT_PROP, "layer": LAYER_PROP,
          ...
          "brush": {"type": "string", "default": "solid",
                     "enum": ["solid", "dashed", "dotted"]},
          "cap": {"type": "string", "default": "round",
                   "enum": ["round", "flat", "square"]}},
         required=["points"]),
    tool("draw_pressure_curve",
         "压感曲线笔刷: points 每点可为 [x,y] 或 [x,y,压力], 压力取 0~100, 不填默认 100(满压). 传压力则按显式压力; 全不传时 pressure_curve=flat(均匀满压默认)/bell(轻轻重). width 最粗, min_width 最细, color, smooth=True 平滑/False 直线. 批量用 strokes.",
         {"document": DOCUMENT_PROP, "layer": LAYER_PROP,
          "points": {"type": "array", "items": {"type": "array"},
                        "description": "[[x,y]] 或 [[x,y,pressure(0~100)]]"},
          "width": {"type": "number", "default": 24.0},
          "min_width": {"type": "number", "default": 1.0},
          "color": {"type": ["string", "array"]},
          "oversample": {"type": "integer", "default": 16},
          "smooth": {"type": "boolean", "default": True},
          "pressure_curve": {"type": "string", "default": "flat",
                              "enum": ["bell", "flat"]},
          ...
          "strokes": {"type": "array", "items": {"type": "object"},
                        "description": "批量: 多条笔触, 每项 {points,width,min_width,color,smooth,pressure_curve}. 与单条 points 二选一."}},
         required=["points"]),
    tool("erase",
